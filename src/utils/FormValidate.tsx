...
export default function validator(state) {
  switch (state.activityType) {
    ...
    case "multiple":
      ...
      if (
        !state.batchDetails.length ||
        state.batchDetails.some((activity) =>
          Object.keys(activity["details"]).length ? false : true
        )
      ) {
      ...
      if (state.activeStep === 1) {
        ...
        let errors = state.batchDetails.map((activity) => {
          ...
          if (cumulativeHours > parseFloat(activity.details.projectHours)) {
            message =
              ...
              ": the total time spent on the activities is bigger than the hours spent on the project";
          }
        ...
        if (errors.some((x) => (x ? true : false))) {
          return (
            errors[0].projectName +
            " " +
            errors[0].activityType +
            ": " +
    ...
    case "Project Support":
      ...
      if (
        state.activeStep === 1 &&
        !/^\d{8}(-\d{2})?$/.exec(state.details["projectNumber"])
      ) {
      ...
      if (
        state.activeStep === 1 &&
        (state.details["taskType"] === "Review" ||
          state.details["taskType"] === "Innovation") &&
        !state.details["reviewInnovationDescription"]
      ) {
        return `Please describe the ${state.details[
          "taskType"
        ].toLowerCase()} ${
          state.details["taskType"] === "Innovation" ? "task" : ""
        }`;
      }
    ...
    case "Client Engagement":
      ...
      if (
        state.activeStep === 1 &&
        (state.details["followup"] == "Yes" ||
          state.details["followup"] == "Maybe")
      ) {
    ...
    case "Presentations and Talks":
      ...
      if (
        state.activeStep === 1 &&
        state.details["presentationType"] === "Other" &&
        !state.details["otherType"]
      ) {
    ...
    case "Continuous Improvement":
      ...
      if (
        state.activeStep === 1 &&
        state.details["continuousImprovementType"] === "Other" &&
        !state.details["otherType"]
      ) {
      ...
      if (
        state.activeStep === 1 &&
        !state.details["continuousImprovementType"]
      ) {
        return "Please specify the activity type";
      }
