...
export class AbstractSubmission {
  ...
  async save(data) {
    ...
    const web = Web(
      `https://${CONFIG.MS_TENANT}.sharepoint.com/sites/${CONFIG.DB_ENDPOINT}`
    );
    console.log(
      `https://${CONFIG.MS_TENANT}.sharepoint.com/sites/${CONFIG.DB_ENDPOINT}`
    );
    try {
      const latestSubmission = await web.lists
        .getByTitle("Submission")
        .items.orderBy("submissionId", false)
        .top(1)
        .get();
    ...
    const submissionData = {
      ...
      activityDate:
        typeof this.activityDate === "string"
          ? this.activityDate
          : format(this.activityDate, "MM/dd/yyyy"),
  ...
  async getPreviousSubmissions() {
    ...
    try {
      const previousSubmissions = await web.lists
        .getByTitle("Submission")
        .items.filter(`staffEmail eq '${currentUser.Email.toLowerCase()}'`)
  ...
  async getActivity(submissionId) {
    ...
    try {
      const activity = await web.lists
        .getByTitle(this.endpoint)
        .items.filter(`submissionId eq '${submissionId}'`)
        .get();

      let cleanActivity = {};
      this.activityCols.map(
        (col) => (cleanActivity[col] = activity[0][col] ? activity[0][col] : "")
      );
      return cleanActivity;
    } catch (e) {
      throw (
        `There was an error in retrieving the activity ${submissionId}: \n` + e
      );
    }
  }
}
...
export class Training extends AbstractSubmission {
  constructor() {
    ...
    this.activityCols = ["trainingMode", "additionalLink"];
  }
}
...
export class Recruitment extends AbstractSubmission {
  constructor() {
    ...
    this.activityCols = ["recruitmentDescription", "additionalLink"];
  }
}
...
export class DeTeamMeeting extends AbstractSubmission {
  constructor() {
    ...
    this.activityCols = ["minutesLink", "meetingDescription", "additionalLink"];
  }
}
...
export class BidSupport extends AbstractSubmission {
  constructor() {
    ...
    this.activityCols = ["inputDescription", "additionalLink"];
  }
}
